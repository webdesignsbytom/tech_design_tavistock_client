...
// Utils
import { portfolioData } from '../../utils/PortfolioDataUtils';

function PortfolioDisplay({ myRef }) {
    ...
    >
      ...
      <section className='grid sm:grid-cols-2 lg:grid-cols-3 gap-8 justify-center md:mx-2 p-4 lg:mx-8 my-auto py-2'>
        {portfolioData.map((item, index) => {
          return (
            <PortfolioItem key={index} item={item} displayInfo={displayInfo} />
          );
        })}
      </section>
    </section>
...
const PortfolioItem = ({ item, displayInfo }) => {
