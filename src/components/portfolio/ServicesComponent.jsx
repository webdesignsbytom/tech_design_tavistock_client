...
function ServicesComponent() {
  ...
  return (
    ...
    >
      ...
      <section className='my-auto grid w-full sm:w-2/3 mx-auto'>
        <div className='my-auto grid lg:grid-cols-3 gap-8 mx-4 lg:mx-8'>
          {servicesData.map((service, index) => (
            <article
              key={index}
              className='bg-alt-colour text-white rounded-xl text-center p-1 w-full h-min'
            >
              <div
                className={`mb-1 border-4 ${service.borderColor} bg-alt border-solid rounded-lg`}
              >
                <h3 className='p-2 font-semibold text-lg'>{service.title}</h3>
              </div>

              <ul
                className={`grid gap-2 p-2 w-full border-4 bg-alt rounded-lg ${service.borderColor} border-solid`}
              >
